...
export const newUserEmailSend = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({
      success: true,
      message:
        "Kayıt başarılı, giriş yapabilmek için mailinize gelen linki tıklayınız",
    });
  } catch (error) {
...
export const createNewUser = async (req, res) => {
  ...
  console.log("ndwandowa");
  try {
    // await User.create({ name, username, email, password });
    res.status(201).json({
      success: true,
      message: "Tebribler! Üyelik aktive edilmiştir, lütfen giriş yapınız",
    });
  } catch (error) {
